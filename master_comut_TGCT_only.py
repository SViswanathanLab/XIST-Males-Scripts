...
figsize = (10,4.5) # the size of the figure - changes the shape of the squares in the comut
...
maf_path = '/Users/ananthansadagopan/Documents/ViswanathanLab/full_TCGA/TGCT_comut_modified_MAF.csv' # change this to the path of your MAF, e.g. 'mutation.maf'
...
output_df.to_csv("/Users/ananthansadagopan/Downloads/temp.csv", index=False)
...
#example_comut.add_continuous_data(nea_df, name = 'NE:A Ratio', mapping = 'Greens', value_range = (-1.04, 1.6))

#example_comut.add_continuous_data(output_df, name = 'NE:E Ratio', mapping = 'Reds', value_range = (-0.7, 1.3))
...
purity_ax = fig.add_axes([0.93, 0.47, 0.06, 0.01])
...
# color bars must be added manually based on figure coordinates - [left, bottom, width, height]
exp_ax = fig.add_axes([0.85, 0.47, 0.06, 0.01])
...
"""
# color bars must be added manually based on figure coordinates - [left, bottom, width, height]
nee_ax = fig.add_axes([0.85, 0.34, 0.06, 0.01])


# purity ranges from 0 to 1
norm = matplotlib.colors.Normalize(vmin=-0.7, vmax=1.3)
...
nee_colorbar.set_ticks([-0.7,0.3,1.3])
nee_colorbar.set_ticklabels([-0.7,0.3,1.3])
nee_colorbar.outline.set_visible(False)

# set title of colorbar to line up with other legend elements
nee_colorbar.set_label('log$_2$(NE:E Ratio)', fontsize = font_size)

output_df.to_csv("/Users/ananthansadagopan/Downloads/output.csv", index=False)


# color bars must be added manually based on figure coordinates - [left, bottom, width, height]
nea_ax = fig.add_axes([0.93, 0.34, 0.06, 0.01])

# purity ranges from 0 to 1
norm = matplotlib.colors.Normalize(vmin=-1.04, vmax=1.6)
...
nea_colorbar.set_ticks([-1,0.3,1.6])
nea_colorbar.set_ticklabels([-1,0.3,1.6])
...
# color bars must be added manually based on figure coordinates - [left, bottom, width, height]
neem_ax = fig.add_axes([0.85, 0.34, 0.06, 0.01])
...
example_comut.figure.savefig('/Users/ananthansadagopan/Documents/ViswanathanLab/XIST_Males/TGCT_master_comut_supplement.pdf', dpi = dpi, bbox_inches = 'tight')
example_comut.figure.savefig('/Users/ananthansadagopan/Documents/ViswanathanLab/XIST_Males/TGCT_master_comut_supplement.png', dpi = dpi, bbox_inches = 'tight')
