...
3) Filter outlier genes by normalized expression (genes with average normalized expression in XIST- females or XIST+ males of more than double or less than half of the appropriate pseudoreference). Filtering by standard deviation can also be performed at this step (run remove_outlier_genes.py on the tumor .csv files).
