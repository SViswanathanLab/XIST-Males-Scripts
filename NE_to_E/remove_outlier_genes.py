...
female_ne = ne_valid_genes['XISTpos_female_ref'].tolist()
male_ne = ne_valid_genes['XISTneg_male_ref'].tolist()

if testing_val == "Female" and gene_type == "Non-Escapee":

    df = df[df['Unnamed: 0'].isin(female_ne)]
    
elif testing_val == "Female" and gene_type == "Escapee":

    df = df[df['Unnamed: 0'].isin(female_e)]

elif testing_val == "Male" and gene_type == "Non-Escapee":

    df = df[df['Unnamed: 0'].isin(male_ne)]
    
elif testing_val == "Male" and gene_type == "Escapee":

    df = df[df['Unnamed: 0'].isin(male_e)] 
...
output_df.to_csv(name + "_speific_genes_stddev_filtered_" + str(cutoff) + ".csv")
...
ax.set_xlabel("Fisher-Pearson Coefficient of Skewness for\nNormalized " + gene_type + " Expression in " + testing_val + " Tumors")
ax.set_ylabel("Number of Samples")

#fig.savefig(name + "_stddev_filtered_" + str(cutoff) + ".pdf", dpi=dpi_set, bbox_inches = 'tight')
