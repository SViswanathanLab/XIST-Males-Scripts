...
def runSimulation(parameter):
        os.system("java -Djava.io.tmpdir=xist_aser -Xmx8000m -jar gatk.jar ASEReadCounter -R Homo_sapiens_assembly38.fasta --read-filter PassesVendorQualityCheckReadFilter --read-filter HasReadGroupReadFilter  --read-filter NotDuplicateReadFilter --read-filter MappingQualityAvailableReadFilter --read-filter NotSecondaryAlignmentReadFilter --read-filter MappingQualityReadFilter --minimum-mapping-quality 30 --read-filter OverclippedReadFilter --filter-too-short 25 --read-filter GoodCigarReadFilter --read-filter AmbiguousBaseReadFilter -V /home/ma1111/GATK_ref/hapmap_3.3.hg38.vcf.gz --lenient --seconds-between-progress-updates 100 -I download_dir/%s/normal.bam -L chrX -O output_dir/%s.out" % (parameter,parameter))
