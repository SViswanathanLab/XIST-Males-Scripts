...
while a<len(shared_lineages):
    ...
    axs[a].set_ylim([0.01, 900])
    axs[a].set_yticks([100])
    
    #xs[a+1].set_ylim([0.01, 500])
...
while (a-len(shared_lineages))<len(uq_male_lineages):
    ...
    df2_temp = df[df['Classification']==uq_male_lineages[a-len(shared_lineages)]]
    ...
    axs[a].hist(female_temp, density=False, bins=bins_list, color="#B7ADCF", linewidth=0.5, edgecolor="none")
    axs[a].set_ylabel("M-" + str(uq_male_lineages[a-len(shared_lineages)]) + " (" + str(len(female_temp)) + ")", rotation=0, labelpad=30, fontsize=temp_font)
...
fig.savefig("/Users/ananthansadagopan/Documents/ViswanathanLab/XIST_Males/MALES_XIST_exp_by_lineage_website_GTEx_male_and_female_histogram.png", dpi=dpi_set, bbox_inches = 'tight')
fig.savefig("/Users/ananthansadagopan/Documents/ViswanathanLab/XIST_Males/MALES_XIST_exp_by_lineage_website_GTEx_male_and_female_histogram.pdf", dpi=dpi_set, bbox_inches = 'tight')
